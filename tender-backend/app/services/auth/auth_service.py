...
class AuthService:
    ...
    async def register(self, data: RegisterRequest) -> TokenResponse:
        ...
        return self._create_tokens(user.id, user.token_version)
    ...
    async def refresh(self, refresh_token: str) -> TokenResponse:
        ...
        # Reject refresh tokens issued before password change or logout-all
    ...
    async def change_password(
        ...
        logger.info("Password changed for user %d, token_version→%d", user_id, new_version)
    ...
    def _create_tokens(self, user_id: int, token_version: int = 1) -> TokenResponse:
        """Create access and refresh token pair embedding token_version."""
        token_data = {"sub": str(user_id), "ver": token_version}
