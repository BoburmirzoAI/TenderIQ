...
from app.schemas.payment import ClickWebhookData, PaymeWebhookData
...
class PaymentService:
    ...
    async def process_click_webhook(self, data: ClickWebhookData) -> dict:
        ...
        if data.sign_string != expected_sign:
    ...
    async def _payme_perform(self, data: PaymeWebhookData) -> dict:
        ...
        order_id = params.get("account", {}).get("order_id")
        if order_id:
            await self._complete_payment(int(order_id), str(params.get("id", "")))
        return {
            "result": {
    ...
    async def _payme_cancel(self, data: PaymeWebhookData) -> dict:
        ...
        return {
            "result": {
                ...
                "transaction": str(data.params.get("id", "")),
                "state": -1,
            },
            "id": data.id,
        }
